...
@testset "Camera" begin
    @testset "IdealCamera Construction" begin
        ...
        @test cam.pixel_edges_x[1] ≈ 0.0
        @test cam.pixel_edges_x[2] ≈ 0.1
        @test eltype(cam.pixel_edges_x) === Float64
        
        # Test rectangular pixels
        cam_rect = IdealCamera(1:512, 1:256, (0.1, 0.2))
        @test length(cam_rect.pixel_edges_x) == 513
        @test length(cam_rect.pixel_edges_y) == 257
        @test cam_rect.pixel_edges_y[2] - cam_rect.pixel_edges_y[1] ≈ 0.2
        
        # Test non-standard ranges
        cam_offset = IdealCamera(2:4, 3:5, 0.1)
        @test length(cam_offset.pixel_edges_x) == 4  # 3 pixels + 1
        @test cam_offset.pixel_edges_x[1] ≈ 0.1  # starts at second pixel
    end
